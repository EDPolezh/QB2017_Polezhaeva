...
1. loaor the bacterial operational taxonomic units (OTUs),  
...
legend("topleft", legend = paste("Saling exponent = ", exponent, sep = ""), 
       bty = "n", lw = 2, col = "red")
```
...
```{r}
ses.pd <- ses.pd(comm[1:2,], phy, null.model = "", runs = 25, include.root = F)
ses.pd <- ses.pd(comm[1:2,], phy, null.model = "", runs = 25, include.root = F)
